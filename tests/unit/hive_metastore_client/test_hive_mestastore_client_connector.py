...
class TestHiveMetastoreClientConnector:
    ...
    @mock.patch("hive_metastore_client.hive_mestastore_client_connector.TBinaryProtocol")
    def test__init_protocol(self,
            ...
            # act
            returned_value = hive_metastore_client._init_protocol(host='111.222.333', port=1234)
    ...
    @mock.patch.object(HiveMetastoreClientConnector, 'open')
    def test__enter__(self, mocked_open, hive_metastore_client):
        # act
        with hive_metastore_client as c:
            pass

        # assert
        mocked_open.assert_called_once_with()

    @mock.patch.object(HiveMetastoreClientConnector, 'close')
